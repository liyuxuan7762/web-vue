<template>
  <div class="pay">
    ...
    <div class="address">
      ...
      <div class="info" v-if="true">
        <div class="info-content">
          <span class="name">{{ getDefaultAddress.name }}</span>
          <span class="mobile">{{ getDefaultAddress.phone }}</span>
        </div>
        <div class="info-address">{{ getLongAddr }}</div>
      </div>
    ...
    <div class="footer-fixed">
      ...
      <div class="tipsbtn" @click="summit">提交订单</div>
    </div>
  </div>
</template>
...
import { createPay, summitOrder } from "@/api/pay";
export default {
  name: "PayIndex",
  data() {
    return {
  ...
  computed: {
    ...mapGetters("address", ["getDefaultAddress", "getLongAddr"]),
    ...mapGetters("cart", [
      "totalCount",
      "totalPrice",
      "isCheckAll",
  ...
  async created() {
    this.getAddressList();
    console.log("默认地址", this.selectedItems);

    if (this.mode === "cart") {
      const {
        data: { order, personal },
      } = await createPay(this.mode, {
        cartIds: this.cartIds,
      });
      this.order = order;
