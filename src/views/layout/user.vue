<template>
  <div class="user">
    ...
    <div v-else class="head-page" @click="$router.push('/login')">
      ...
      <div class="info">
        <div class="mobile">{{ detail.nick_name || "未登录" }}</div>
        <div v-if="!detail.nick_name" class="words">点击登录账号</div>
      </div>
    </div>
    ...
    <div class="my-asset">
      <div class="asset-left">
        ...
        <div class="asset-left-item">
          <span>{{ detail.coupon_money || 0 }}</span>
          <span>优惠券</span>
        </div>
      </div>
    ...
    <div class="logout-btn">
      <button @click="logout">{{ isLogin ? "退出登录" : "去登录" }}</button>
    </div>
  </div>
</template>
